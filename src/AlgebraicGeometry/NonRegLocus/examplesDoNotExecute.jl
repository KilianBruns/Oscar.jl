...
R,(x, y, z, v, w) = polynomial_ring(QQ, ["x", "y", "z", "v", "w"])
R,(x, y, z, v, w) = polynomial_ring(ZZ, ["x", "y", "z", "v", "w"])
IZ = ideal(R, zero(R))
IX = IZ + ideal(R, [x, v, w, y^2 - z^2])
is_regular(IZ, IX)
non_regular_locus(IZ, IX)
...
IX = ideal(R, [(8c+4)*x^2*y^2*z^2 - c^4*(x^4*y^2 + y^4*z^2 + x^2*z^4) + c^2*(x^2*y^4 * y^2*z^4 * x^4*z^2) - (2*c + 1)//4*(x^2 + y^2 + z^2 - 1)^2])
